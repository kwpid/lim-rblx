...
## Recent Changes
- **2025-11-03**: Updated TradeClient value displays - Trade values now show in shortened format (e.g., "4.6M" instead of "48500000") for better readability. Both "Your offer" and "Their offer" value displays use the new format.
- **2025-11-03**: Added new "Scavenger Hunt" event - A single Ultra Epic or higher rarity item (excluding face items) is hidden at a random location in workspace.ScavengerEvent. Uses weighted probability with exponent 2.5, heavily favoring mid-tier Ultra Epic items (750k-2.5M), making Mythic items rare and Insane items extremely rare. The item is anchored at the location with proximity prompt and very dim spotlight (brightness 0.1-0.3, range 5 studs). Event has 5-minute timeout - if nobody finds the item within 5 minutes, the event automatically ends. When a player finds and collects the item, they receive a collection notification and the event immediately ends with a server-wide notification showing who found it and what item it was. Stock items are supported with serial number claiming.
- **2025-11-03**: Updated EventSystem to prevent multiple events from running simultaneously - The random event spawner now checks if any event is active before starting a new one. Manual StartEvent() calls are also blocked if another event is running. EventTextHandler already exists in StarterGUI to display "Event Ongoing!" in the MainUI.
- **2025-11-02**: Fixed critical trading quantity bug - Quantity items weren't being properly removed from trader's inventory during trades. The code was modifying a copy of the item instead of the actual inventory item. Now correctly updates the actual inventory by accessing senderData.Inventory[inventoryIndex] directly, and properly decrements TotalCopies and Owners in ItemDatabase during trades.
...
-   **Event System**: Dynamic, modular event system with automatic spawning (5-10 minute intervals) and manual admin triggers. Only one event can be active at a time - the system prevents overlapping events. Two events are currently available: (1) "Random Item Drops" spawns multiple items over time at a dropzone with rarity-based multipliers favoring rare items and ends after all items drop, and (2) "Scavenger Hunt" hides a single Ultra Epic+ item (excluding faces) at a random location in workspace.ScavengerEvent with a 5-minute timeout - if nobody finds it within 5 minutes, the event ends automatically. The hidden item has a very dim spotlight (brightness 0.1-0.3, range 5 studs) making it harder to spot. Event ends immediately when collected with server-wide notification of the winner. All events use actual Roblox item models with rarity-colored highlights and proximity prompts. EventTextHandler displays "Event Ongoing!" in MainUI during active events.
