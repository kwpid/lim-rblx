...
-   **Event System**: Dynamic, modular event system with automatic spawning (5-10 minute intervals) and manual admin triggers. Only one event can be active at a time - the system prevents overlapping events. Two events are currently available: (1) "Random Item Drops" spawns multiple items over time at a dropzone with rarity-based multipliers favoring rare items and ends after all items drop, and (2) "Scavenger Hunt" hides a single Ultra Epic+ item (excluding faces) at a random location in workspace.ScavengerEvent and continues indefinitely until someone finds it - the event ends immediately when collected with server-wide notification of the winner. All events use actual Roblox item models with rarity-colored highlights and proximity prompts. EventTextHandler displays "Event Ongoing!" in MainUI during active events.
