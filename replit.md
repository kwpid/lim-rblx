...
-   **Crate Opening**: Features weighted random selection using inverse value formula (value^1.1) for drop rates, visual animations, camera shake effects (intensity scales with rarity), serial number display, "Fast Roll," "AutoRoll," and "HideRolls" options. High-value unboxes trigger server-wide notifications. Limited and Vanity items are not rollable via crates.
...
-   **Risk It 1v1 Gambling System**: Player-versus-player gambling game where players bet items and compete until one player reaches 7 wins (first to 7). Features include: request/accept flow (similar to trading), item selection with value matching (25% tolerance), animated spin rounds using random items from the database, winner-takes-all item transfers, and cancellation support. Players select items from their inventory (excluding Vanity and Common items), both players must confirm their selections, and values must be within 25% of each other. Each round, both players spin for random items - highest value wins that round. First player to reach 7 wins takes all items from both sides. **Anti-Cheat**: If a player leaves mid-game after the game has started, the remaining player automatically wins and receives all items from both sides. Data is saved immediately. If a player leaves before the game starts, the gamble is simply cancelled. **Configuration**: `ReplicatedStorage/GambleConfig.lua` controls which rarities are allowed (`AllowCommons` always false, `AllowUncommons` toggleable). **Quantity Handling**: Stacked items are displayed as individual selectable items - if a player owns 3 fedoras, they appear as 3 separate selections. Server validates that players cannot gamble more copies than they own. **Serial Number Preservation**: Serial numbers are properly preserved when transferring items between winner and loser. **Game Reset**: After each game completes, the UI is completely reset (wins counters, selected items, button states, connections) to ensure clean state for the next game. **Data Integrity**: Player data is automatically saved to DataStore after each game completes (both winner and loser). Managed by `GambleServer.lua` (server) and `GambleClient.lua` (client) with integration into the unified tab system via `TabManager.lua`.
...
-   Calculates and displays seller's actual payout (after 15% tax)