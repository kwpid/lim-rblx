...
-   **Risk It 1v1 Gambling System**: Player-versus-player gambling game where players bet items and compete in 7 rounds to win the opponent's items. Features include: request/accept flow (similar to trading), item selection with value matching (25% tolerance), animated spin rounds using random items from the database, winner-takes-all item transfers, and cancellation support. Players select items from their inventory (excluding Vanity and Common items), both players must confirm their selections, and values must be within 25% of each other. The game consists of 7 rounds where both players spin for random items - highest value wins each round. Player with most round wins takes all items from both sides. **Configuration**: `ReplicatedStorage/GambleConfig.lua` controls which rarities are allowed (`AllowCommons` always false, `AllowUncommons` toggleable). **Quantity Handling**: Stacked items are displayed as individual selectable items - if a player owns 3 fedoras, they appear as 3 separate selections. Server validates that players cannot gamble more copies than they own. **Serial Number Preservation**: Serial numbers are properly preserved when transferring items between winner and loser. Managed by `GambleServer.lua` (server) and `GambleClient.lua` (client) with integration into the unified tab system via `TabManager.lua`.
