...
-   **Event System**: Dynamic, modular event system with automatic spawning (5-10 minute intervals) and manual admin triggers. Two events are currently available: (1) "Random Item Drops" spawns multiple items over time at a dropzone with rarity-based multipliers favoring rare items, and (2) "Scavenger Hunt" hides a single Ultra Epic+ item (excluding faces) at a random location in workspace.ScavengerEvent - the event ends immediately when someone finds it with server-wide notification of the winner. All events use actual Roblox item models with rarity-colored highlights and proximity prompts.
