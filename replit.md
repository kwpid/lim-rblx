...
## Recent Changes
- **2025-11-02**: Fixed search functionality in ViewInventory UI - the search bar now properly filters items when viewing other players' inventories. Previously, the search connection was set up before buttons were created, causing it to reference an empty array. Now the search connection is created each time an inventory is loaded.
...
- **2025-11-02**: Fixed serial owner preservation during trades - when a serial item is traded, the original owner is now preserved in the database. For example, if Player1 trades serial #25 to Player2, the item database will still show Player1 as the original owner of serial #25, even though Player2 now owns it.
