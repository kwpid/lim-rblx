...
## Recent Changes
- **2025-11-03**: Added new "Scavenger Hunt" event - A single Ultra Epic or higher rarity item (excluding face items) is hidden at a random location in workspace.ScavengerEvent. Uses weighted probability with exponent 1.5, making Ultra Epic items most common and Insane items extremely rare. The item is anchored at the location with proximity prompt and very dim spotlight (brightness 0.1-0.3, range 5 studs). Event has 5-minute timeout - if nobody finds the item within 5 minutes, the event automatically ends. When a player finds and collects the item, they receive a collection notification and the event immediately ends with a server-wide notification showing who found it and what item it was. Stock items are supported with serial number claiming.
