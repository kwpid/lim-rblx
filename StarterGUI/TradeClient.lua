...
local re = tradeRS:WaitForChild("RemoteEvent")
local config = require(ReplicatedStorage:WaitForChild("TradeConfiguration"))

local tradeRequestsFolder = tradeRS:WaitForChild("TRADE REQUESTS")
local ongoingTradesFolder = tradeRS:WaitForChild("ONGOING TRADES")
...
ongoingTradesFolder.ChildAdded:Connect(function(child)
  if child:WaitForChild("Sender").Value == client.Name or child:WaitForChild("Receiver").Value == client.Name then
    ...
    for i, child in pairs(tradeFrame.TradingFrame.YourOfferFrame.Slots:GetChildren()) do
      if child:IsA("TextButton") or child:IsA("ImageButton") or child:IsA("Frame") then
        child:Destroy()
      end
    end
    for i, child in pairs(tradeFrame.TradingFrame.TheirOfferFrame.Slots:GetChildren()) do
      if child:IsA("TextButton") or child:IsA("ImageButton") or child:IsA("Frame") then
        child:Destroy()
      end
    end
    ...
    for _, displayItem in ipairs(itemsToDisplay) do
      ...
      newItemButton.MouseButton1Click:Connect(function()
        if displayItem.isSerial then
          local alreadyInTrade = tradeFrame.TradingFrame.YourOfferFrame.Slots:FindFirstChild(uniqueId)
          if alreadyInTrade then
            re:FireServer("remove item from trade", {displayItem.RobloxId, displayItem.SerialNumber})
          else
            re:FireServer("add item to trade", {displayItem.RobloxId, displayItem.SerialNumber})
          end
        else
          if displayItem.Amount < displayItem.MaxAmount then
            ...
            re:FireServer("add item to trade", {displayItem.RobloxId, nil, 1})
          end
        end
      end)
      ...
      newItemButton.MouseButton2Click:Connect(function()
        if not displayItem.isSerial then
          if displayItem.Amount > 0 then
            ...
            re:FireServer("remove item from trade", {displayItem.RobloxId, nil, 1})
          end
        end
      end)
    ...
    clientOffer.ChildAdded:Connect(function(slotChild)
      ...
      newToolButton.Name = slotChild.Name
      ...
      newToolButton.MouseButton1Click:Connect(function()
        if serialNumber then
          re:FireServer("remove item from trade", {robloxId.Value, serialNumber.Value})
        else
          re:FireServer("remove item from trade", {robloxId.Value, nil, 1})
        end
      end)
