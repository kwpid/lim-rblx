...
local function updateValueDisplays()
  ...
  if currentTrade then
    local clientValue = currentTrade:FindFirstChild("Sender") and currentTrade.Sender.Value == client.Name and
        currentTrade.Sender or currentTrade.Receiver
...
local function createItemButton(item, isForInventory)
  ...
  if displayInfo.useViewport then
    ...
    if not newToolButton.ItemViewportFrame:FindFirstChild("QuantityLabel") then
      ...
      quantityLabel.Visible = (isForInventory and (totalAmount - tradeAmount) > 1) or
          (not isForInventory and tradeAmount > 1)
    end
  else
    -- Use image for faces
    ...
    clientOffer.ChildAdded:Connect(function(child)
      ...
      if tradeAmountValue then
        tradeAmountValue.Changed:Connect(function()
          ...
          local quantityLabel = newToolButton.ItemViewportFrame:FindFirstChild("QuantityLabel") or
              newToolButton.ItemImageLabel:FindFirstChild("QuantityLabel")
...
local function createHistoryEntry(historyData)
  ...
  local success, playerImage = pcall(function()
    return game.Players:GetUserThumbnailAsync(historyData.tradedWithUserId, Enum.ThumbnailType.HeadShot,
      Enum.ThumbnailSize.Size100x100)
  end)
  if success then
