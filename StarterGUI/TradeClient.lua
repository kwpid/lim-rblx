-- TradeClient.lua
-- Rewritten trading client for crate opening game
-- Works with new TradeServer and DataStoreAPI
...
-- Wait for Trade system to be set up
...
-- Store player's tradeable inventory (received from server)
...
-- Helper: Calculate offer value from trade folder
...
-- Helper: Get item from tradeable inventory
local function getItemFromInventory(robloxId, serialNumber)
...
-- Helper: Get amount in current trade
...
local function itemMatchesSearch(item, query)
	...
	return string.find(itemName, searchLower, 1, true) ~= nil
end

-- Helper: Sort items by value
...
local function createItemButton(item, isForInventory)
	...
	if isForInventory then
		if item.SerialNumber then
			newButton.ItemName.Text = item.Name .. " #" .. item.SerialNumber
		elseif availableAmount > 1 then
			newButton.ItemName.Text = item.Name .. " x" .. availableAmount
		else
		...
		elseif tradeAmount > 1 then
			newButton.ItemName.Text = item.Name .. " x" .. tradeAmount
		else
			newButton.ItemName.Text = item.Name
		end
	end
	...
	-- Set thumbnail image
	if newButton:FindFirstChild("ItemImageLabel") then
		newButton.ItemImageLabel.Image = "rbxassetid://" .. item.RobloxId
	end
	
	-- Show if in trade
	if newButton:FindFirstChild("IsInTrade") then
		newButton.IsInTrade.Visible = (tradeAmount > 0) and isForInventory
	end
	
	return newButton
end

-- Update inventory display in trade window
...
local function updateTradeOffersDisplay()
	...
	local accepted = currentTrade.Sender.Value == client.Name and currentTrade.Sender:FindFirstChild("ACCEPTED") or currentTrade.Receiver:FindFirstChild("ACCEPTED")
...
if sendTradesFrame:FindFirstChild("PlayerList") then
	for _, playerButton in pairs(sendTradesFrame.PlayerList:GetChildren()) do
...
print("✅ TradeClient loaded successfully")
