...
local sendNotificationEvent = remoteEvents:FindFirstChild("CreateNotification") or Instance.new("RemoteEvent")
...
function EventSystem:StartEvent(eventName)
  ...
  if not eventModule then return false end
  for _, active in pairs(self.ActiveEvents) do
    if active then return false end
  end
  local success, eventInfo = pcall(function() return eventModule.GetEventInfo() end)
  ...
  task.spawn(function()
    while true do
      ...
      if not hasActiveEvent then
        local eventNames = {}
        for eventName in pairs(self.EventModules) do
